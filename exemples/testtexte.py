...
class Buffer(gtk.TextBuffer):
    ...
    def remplir_buffer_exmpl(self):
        ...
        for i in range(100):
            ...
            self.insert(iter,
                          "(Salut tout le monde !)\nñaca ñaca Salut, voilà du texte pour tester le retour à la ligne automatique, et avec de la ponctuation et accents s'il vous plaît ! Super, bla-bla - hmm, ouais.\nallez, une nouvelle ligne avec pas mal de texte. Y'a vraiment pas mal de texte... Du texte ! Encore du texte !\n"
                          "Allemand (Deutsch Süd) Grüß Gott Grec (Ελληνικά) Γειά σας Hébreu(שלום) Ponctuation hébraïque(\xd6\xbfש\xd6\xbb\xd6\xbc\xd6\xbb\xd6\xbfל\xd6\xbcו\xd6\xbc\xd6\xbb\xd6\xbb\xd6\xbfם\xd6\xbc\xd6\xbb\xd6\xbf) Japonais (日本語) Thaï (สวัสดีครับ) Caractères thaïs (คำต่อไปนื่สะกดผิด พัั้ัั่งโกะ)\n")
            ...
            self.insert(iter, "Droite->gauche + retour à la ligne auto\n")
